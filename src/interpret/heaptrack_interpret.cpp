...
bool isArmArch()
{
    // TODO: this should be part of the recorded trace data, as we may want to run heaptrack_interpret
    //       on a secondary machine running a different architecture in the future
#if __arm__
    return true;
#else
    return false;
...
struct Module
{
    ...
    AddressInformation resolveAddress(uintptr_t address) const
    {
        ...
        if (!symbolCache->hasSymbols(fileName)) {
            ...
            symbolCache->setSymbols(fileName, SymbolCache::extractSymbols(module, addressStart, isArmArch()));
        }
        ...
        auto srcloc = findFileAndLine(cuDie->cudie(), offset);
        if (srcloc) {
            info.frame.file = std::move(srcloc.file);
            info.frame.line = srcloc.line;
        }
        ...
        auto handleDie = [&](Dwarf_Die scope) {
            ...
            info.inlined.emplace_back(cuDie->dieName(&scope), std::move(file), line);
        };
